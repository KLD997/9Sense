...
Rectangle	viewr;
int		threadrforkflag = 0;	/* should be RFENVG but that hides 9sense from plumber */
...
usage(void)
{
	fprint(2, "usage: 9sense [-b] [-f font] [-i initcmd] [-k kbdcmd] [-s]\n");
	exits("usage");
}
...
threadmain(int argc, char *argv[])
{
	...
	if(fontname){
		...
		if(access(fontname, 0) < 0){
			fprint(2, "9sense: can't access %s: %r\n", fontname);
			exits("font open");
		}
	...
	if(geninitdraw(nil, derror, nil, "9sense", nil, Refnone) < 0){
		fprint(2, "9sense: can't open display: %r\n");
		exits("display open");
	}
	iconinit();
	...
	if(filsys == nil)
		fprint(2, "9sense: can't create file system server: %r\n");
	else{
...
initcmd(void *arg)
{
	...
	fd = open(wsys, ORDWR);
	if(fd < 0)
		fprint(2, "9sense: failed to open wsys: %r\n");
	if(mount(fd, -1, "/mnt/wsys", MREPL, "none") < 0)
		fprint(2, "9sense: failed to mount wsys: %r\n");
	if(bind("/mnt/wsys", "/dev/", MBEFORE) < 0)
		fprint(2, "9sense: failed to bind wsys: %r\n");
	free(wsys);
	close(fd);
	procexecl(nil, "/bin/rc", "rc", "-c", cmd, nil);
	fprint(2, "9sense: exec failed: %r\n");
	exits("exec");
}
