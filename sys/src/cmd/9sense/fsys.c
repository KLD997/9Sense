...
filsysinit(Channel *cxfidalloc)
{
	...
	snprint(srvpipe, sizeof(srvpipe), "/srv/9sense.%s.%lud", fs->user, (ulong)getpid());
...
filsysproc(void *arg)
{
	...
	for(;;){
		...
		if(n <= 0){
			...
			fprint(2, "9sense: %d: read9pmsg: %d %r\n", getpid(), n);
			errorshouldabort = 0;
...
filsysauth(Filsys *fs, Xfid *x, Fid*)
{
	...
	return filsysrespond(fs, x, &t, "9sense: authentication not required");
}
